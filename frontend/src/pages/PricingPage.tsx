  ...
  const plans = [
    {
      ...
      description: t('Perfect pentru a începe', 'Perfect to get started', 'Perfetto per iniziare'),
      features: [
    ...
    },
    {
      ...
      description: t('Pentru afaceri mici și medii', 'For small and medium businesses', 'Per piccole e medie imprese'),
      features: [
        { text: t('1 locație', '1 location', '1 sede'), included: true },
    ...
    },
    {
      name: 'Professional',
      price: 60,
      yearlyPrice: 576, // 20% discount
      badge: t('Avansat', 'Advanced', 'Avanzato'),
      gradient: 'from-blue-500 to-cyan-600',
      glowColor: 'rgba(59,130,246,0.25)',
      borderColor: 'rgba(59,130,246,0.35)',
      description: t('Pentru catene și francize', 'For chains and franchises', 'Per catene e franchising'),
      features: [
    ...
    },
    {
      ...
      description: t('Pentru afaceri la scară mare', 'For large-scale businesses', 'Per grandi aziende'),
      features: [
    ...
    <div>
      ...
      <div className="relative w-full overflow-hidden">
        ...
        <div className="relative z-20">
          ...
          <button
            onClick={() => navigate('/')}
            className="fixed top-6 left-6 z-50 flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl transition-all duration-300 backdrop-blur-sm border border-white/20"
          >
          ...
          <div className="max-w-7xl mx-auto px-6 py-16 text-center">
            ...
            <div className="flex items-center justify-center gap-4 mb-14">
              <span className={`text-lg font-semibold transition-colors ${billingCycle === 'monthly' ? 'text-white' : 'text-white/40'}`}>
                {t('Lunar', 'Monthly', 'Mensile')}
              </span>
              <button
                onClick={() => setBillingCycle(billingCycle === 'monthly' ? 'yearly' : 'monthly')}
                className="relative w-16 h-8 rounded-full transition-colors duration-300"
                style={{ background: billingCycle === 'yearly' ? 'rgba(139,92,246,0.8)' : 'rgba(255,255,255,0.2)' }}
              >
              ...
              <span className={`text-lg font-semibold transition-colors flex items-center gap-2 ${billingCycle === 'yearly' ? 'text-white' : 'text-white/40'}`}>
                {t('Anual', 'Yearly', 'Annuale')}
                <span className="px-2.5 py-1 bg-emerald-500 text-white text-xs font-bold rounded-full shadow-lg shadow-emerald-500/30">
                  {t('Economisești 20%', 'Save 20%', 'Risparmia 20%')}
                </span>
              </span>
            </div>
                ...
                <div
                  ...
                  style={{
                    background: `
                      ...
                      linear-gradient(160deg, rgba(255,255,255,0.07) 0%, rgba(0,0,0,0.25) 100%)
                    `.replace(/\s+/g, ' '),
                    border: `1px solid ${plan.borderColor}`,
                    boxShadow: plan.popular
                      ? `0 0 0 2px ${plan.borderColor}, 0 20px 60px ${plan.glowColor}, 0 4px 20px rgba(0,0,0,0.4)`
                      : `0 8px 32px rgba(0,0,0,0.3), 0 0 0 1px ${plan.borderColor}`,
                    backdropFilter: 'blur(16px)',
                  }}
                >
                  {/* Top gradient band */}
                  <div
                    className={`relative h-1.5 w-full bg-gradient-to-r ${plan.gradient}`}
                    style={{ boxShadow: `0 0 12px ${plan.glowColor}` }}
                  />

                  {/* Badge */}
                  <div className={`py-2 px-4 text-center bg-gradient-to-r ${plan.gradient} bg-opacity-20`}>
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <p className="text-sm text-white/45 mb-5">{plan.description}</p>
                    ...
                    <div className="mb-6">
                      <div className="flex items-baseline justify-center gap-1">
                        ...
                        {plan.price > 0 && (
                          <span className="text-white/40 text-sm">/{t('lună', 'mo', 'mese')}</span>
                        )}
                      </div>
                    ...
                    <button
                      ...
                      className={`w-full py-3 px-6 rounded-xl font-semibold transition-all duration-300 text-sm ${
                        plan.popular
                      ...
                      style={plan.popular ? { boxShadow: `0 8px 24px ${plan.glowColor}` } : {}}
                    >
                      {plan.cta}
                    </button>
                  </div>
